...
if [ "$(id -u)" != "0" ]; then
    mkdir -p /downloads/albums /downloads/singles /downloads/.incomplete 2>/dev/null || true
    exec "$@"
fi
...
mkdir -p /config /downloads/albums /downloads/singles /downloads/.incomplete
...
# never recursively.
for dir in /downloads /downloads/albums /downloads/singles /downloads/.incomplete; do
